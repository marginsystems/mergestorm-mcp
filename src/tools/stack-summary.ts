import { stackBlockers, stackBlockersSummary, type MergeQueueEntryDto, type StackDto } from "mergestorm/client";

export function stackSummary(stack: StackDto, entries: MergeQueueEntryDto[] = []): string {
  ...
  const { attention, issues } = stackBlockers(stack, entries);
  return `${stack.owner}/${stack.repo} · ${layerLabel} · ${currentState} · auto-land ${
    stack.autoEnqueueWhenReady ? "on" : "off"
  }${overrides.map((label) => ` · ${label}`).join("")}${stackBlockersSummary(attention, issues)}`;
}
